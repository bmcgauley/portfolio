...
import { getCertificationById } from "@/lib/certifications-db";
...
export default async function EditCertificationPage({
  ...
  const update = updateCertificationAction.bind(null, id);

  return (
    <div>
      <header className="mb-12">
        <p className="font-mono uppercase tracking-[0.22em] text-[10px] text-gold-shadow mb-3">
          Office &middot; About &middot; Certifications
        </p>
      ...
      <form action={update} className="space-y-6 max-w-2xl">
        <div>
          ...
          <input id="title" name="title" type="text" required defaultValue={item.title} className={INPUT} />
        </div>
        <div>
          <label htmlFor="issuer" className={LABEL}>Issuer</label>
          <input id="issuer" name="issuer" type="text" required defaultValue={item.issuer} className={INPUT} />
        </div>
        <div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/admin/about#certifications" className="font-display uppercase tracking-[0.18em] text-xs text-ink-soft hover:text-crimson-deep">
            Cancel
          </Link>
        </div>
      </form>
