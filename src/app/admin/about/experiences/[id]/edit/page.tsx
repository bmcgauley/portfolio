...
import { getExperienceById } from "@/lib/experiences-db";
      ...
      <form action={update} className="space-y-6 max-w-2xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input id="position" name="position" type="text" required defaultValue={item.position} className={INPUT} />
          </div>
          <div>
            <label htmlFor="company" className={LABEL}>Company</label>
            <input id="company" name="company" type="text" required defaultValue={item.company} className={INPUT} />
          </div>
          <div>
            <label htmlFor="startDate" className={LABEL}>Start date</label>
            <input id="startDate" name="startDate" type="text" required defaultValue={item.startDate} className={INPUT} />
          </div>
          <div>
            <label htmlFor="endDate" className={LABEL}>End date (leave blank for present)</label>
            <input id="endDate" name="endDate" type="text" defaultValue={item.endDate ?? ""} className={INPUT} />
          </div>
        </div>
        <div>
          <label htmlFor="description" className={LABEL}>Description (markdown supported)</label>
          <textarea id="description" name="description" required defaultValue={item.description} className={`${INPUT} min-h-[160px]`} />
        </div>
        <div>
